...
const DAILY_PLAY_LIMIT = Number(import.meta.env.VITE_DAILY_PLAY_LIMIT ?? 3) || 3
const QUESTION_TIME_LIMIT = Number(import.meta.env.VITE_QUESTION_TIME_LIMIT_MS ?? 20000) || 20000

const SHOP_ITEM = {
  id: 'billiards-card',
  name: 'Billiards Night Gift Card',
  description: 'Redeem for a free game of billiards for you and a friend.',
  instructions: 'Show this animated hologram at the desk to redeem your table time.',
  codePrefix: 'CUE',
}
...
function buildClaimCode(prefix) {
  ...
  return `${prefix}`.toUpperCase().slice(0, 4) + '-' + chunk.toString().padStart(6, '0')
}

function buildShareUrl(token) {
  if (typeof window === 'undefined') return `https://example.com/#/claim/${token}`
  return `${window.location.origin}/#/claim/${token}`
}

function evaluatePlayWindow(log = []) {
  const now = Date.now()
  const filtered = (log || []).filter((timestamp) => now - timestamp < DAY_MS)
...
function sanitizeRun(raw) {
  if (!raw) return null
  const run = { ...raw }
  run.selectedCategoryIds = Array.isArray(run.selectedCategoryIds) ? run.selectedCategoryIds : []
  run.remainingCategoryIds = Array.isArray(run.remainingCategoryIds)
    ? run.remainingCategoryIds
    : [...run.selectedCategoryIds]
  run.roundPointer = Number.isInteger(run.roundPointer) ? run.roundPointer : 0
  ...
  run.categoryResults = run.categoryResults || Object.fromEntries(run.selectedCategoryIds.map((id) => [id, 'unplayed']))
  run.categoriesPlayed = Array.isArray(run.categoriesPlayed) ? run.categoriesPlayed : []
...
function cloneRun(run) {
  ...
  return {
    ...
    questionHistory: Object.fromEntries(
      Object.entries(run.questionHistory).map(([key, value]) => [key, [...value]])
    ),
    ...
    currentSession: run.currentSession
      ...
      : null,
    currentQuestion: run.currentQuestion ? { ...run.currentQuestion } : null,
...
function pickOffer(run, count) {
  ...
  const { items, cursor } = takeRandomItems(available, amount, run.randSeed, run.randCursor || 0)
...
function pickQuestionBatch(run, category) {
  ...
  const { items, cursor } = shuffleItems(category.questions, run.randSeed, run.randCursor || 0)
  run.randCursor = cursor
  ...
  if (batch.length < QUESTION_BATCH_SIZE) {
    ...
    while (extended.length < QUESTION_BATCH_SIZE) {
      extended.push(category.questions[extended.length % category.questions.length])
    }
    return extended
  }
  return batch
...
function shuffleQuestionChoices(run, question) {
  ...
  const decorated = question.choices.map((choice, index) => ({ choice, index }))
  const { items, cursor } = shuffleItems(decorated, run.randSeed, run.randCursor || 0)
  run.randCursor = cursor
  const choices = items.map((entry) => entry.choice)
  const answerIndex = items.findIndex((entry) => entry.index === question.answerIndex)
  return { ...question, choices, answerIndex: answerIndex === -1 ? 0 : answerIndex }
}
...
function createEngine() {
  ...
  function beginSession(run, categoryId) {
    ...
    const preparedQuestions = questions.map((question) => shuffleQuestionChoices(run, question))
    run.currentSession = {
      categoryId,
    ...
    run.currentQuestion = { ...preparedQuestions[0], id: preparedQuestions[0].id }
  ...
  function completeSession(run) {
    ...
    run.remainingCategoryIds = run.remainingCategoryIds.filter((id) => id !== session.categoryId)
  ...
  function handleAnswer(choiceIndex, timedOut = false) {
    updateRun((run) => {
      ...
      run.questionTimer = null
      clearLocks()
      if (questionNumber >= QUESTION_BATCH_SIZE) {
        completeSession(run)
      }
      return true
    })
  ...
  function proceedAfterAnswer() {
    updateRun((run) => {
      ...
      const nextQuestion = run.currentSession.questions[run.currentSession.index]
  ...
  function startRunFromPool() {
    ...
    store.update((state) => {
      const { pruned, closedUntil } = evaluatePlayWindow(state.playLog || [])
      if (isClosed(closedUntil)) {
        persist(state.run, state.purchases, pruned, state.prizeClaimedAt)
        const reopen = new Date(closedUntil).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        return {
          ...state,
          playLog: pruned,
          closedUntil,
      ...
      const { items: selected, cursor } = takeRandomItems(context.questionBank.categories, 7, seed, 0)
      ...
      const run = sanitizeRun({
        ...
        categoriesPlayed: [],
        categoryResults: Object.fromEntries(selectedIds.map((id) => [id, 'unplayed'])),
        questionHistory: {},
        winsCount: 0,
        finished: false,
        prizeUnlocked: false,
  ...
  function loadQuestions() {
    store.update((state) => ({ ...state, questionStatus: 'loading', questionError: null }))
    fetchQuestionBank()
      .then((bank) => {
        const map = {}
      ...
      .catch((error) => {
        store.update((state) => ({
          ...
          questionError: { message: error.message, details: error.details || [] },
          screen: 'error',
        }))
      })
  }
  ...
  function confirmWheelReveal() {
    updateRun((run) => {
      ...
      if (run.currentOffer.type === 'forced') {
        ...
        run.remainingCategoryIds = run.remainingCategoryIds.filter((id) => id !== categoryId)
  ...
  function chooseCategory(categoryId) {
    updateRun((run) => {
      if (!run.currentOffer || run.currentOffer.type !== 'choice') return false
  ...
  function handleNavigationViolation() {
    ...
    if (state.run && state.run.currentStep === 'question') {
      invalidateRun('Navigation detected during a question. Run invalidated.')
    }
  }
  ...
  function handleBackgrounding() {
    const state = get(store)
    if (state.run && state.run.currentStep === 'question') {
      invalidateRun('Question interrupted while the tab was hidden. Run invalidated.')
    }
  }
  ...
  function purchasePrize(prizeId) {
    ...
    store.update((state) => {
      if (!state.run || !state.run.prizeUnlocked) {
        return { ...state, message: 'Win 4 categories to unlock the gift card.' }
      }
      if (prizeUnavailable(state)) {
        return { ...state, message: 'The billiards gift card has already been claimed.' }
      }
      if (state.purchases.length > 0) {
        return { ...state, message: 'You already generated the gift card.' }
      }
    ...
    store.update((state) => {
      ...
      const nextModal =
        state.modal?.type === 'claim'
          ? { ...state.modal, purchase: purchases.find((p) => p.purchaseId === purchaseId) || null }
          : state.modal
  ...
  function openClaim(token) {
    if (!token) return
    store.update((state) => {
      const purchase = state.purchases.find((item) => item.claimToken === token)
      return { ...state, modal: { type: 'claim', token, purchase: purchase || null } }
    })
  }

  function viewPrizeCard(purchaseId) {
    store.update((state) => {
      const purchase = state.purchases.find((item) => item.purchaseId === purchaseId)
  ...
  function initialize() {
    ...
    const { pruned, closedUntil } = evaluatePlayWindow(persisted.playLog || [])
    store.update((state) => ({
      ...state,
      run,
    ...
    if (lock && run && !run.finished) {
      invalidateRun('Refresh detected during a question. Run invalidated.')
      clearLocks()
    }
    loadQuestions()
  }
